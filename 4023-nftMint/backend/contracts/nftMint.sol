...
// (To be a valid NFT, the smart contract must implement all the methods of the ERC-721 standard.)
...
// so only the owner of the smart contract can mint NFTs.

import "@openzeppelin/contracts/access/Ownable.sol";

// @openzeppelin/contracts/utils/Counters.sol provides counters that can only be incremented or decremented by one. 
// The smart contract uses a counter to keep track of the total number of NFTs minted and sets the unique ID on our new NFT.
// (Each NFT minted using a smart contract must be assigned a unique ID—here our unique ID is just determined by the total
// number of NFTs in existence. For example, the first NFT we mint with our smart contract has an ID of "1," 
// our second NFT has an ID of "2," etc.)
...
contract nftMint is ERC721, Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    constructor(string memory baseURI) ERC721("MOD", "MODNFT") {
        setBaseURI(baseURI);
    } 

    function mintNFTs(uint256 _count) public payable {
        uint totalMinted = _tokenIds.current();
        require(totalMinted.add(_count) <= maxSupply, "No more NFTs remaining to mint.");
        require(msg.value >= cost.mul(_count), "Not enough ETH in wallet to mint NFT.");
        if (msg.sender != owner()) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0
            ...
            // at the moment this only handles the same metadata
